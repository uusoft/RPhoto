...
public class UserTagControll extends HttpServlet {
	...
	public void doGetTagCloud(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		PrintWriter out = response.getWriter();
		...
		List<Tag> tags = dao.getAllTags(Integer.parseInt(count));
		...
		while(iter.hasNext()){
			Tag tag = iter.next();
			out.println("<a class=\"tag-item "+(classes[(int)(Math.random()*4)])+"\" href=\"#\">"+tag.getName()+"</a>");
		}
		out.flush();
		out.close();
	}
	...
	public boolean isPhotoBelongToLoginUser(HttpServletRequest request,int photoId){
		...
		if(photo==null||photo.getPid()<=0) return false;
		
		int albumId = photo.getAid();
		AlbumDao adao = new AlbumDaoImpl();
		Album album = adao.getAlbumById(albumId);
		if(album.getUid()==user.getUid()) return true;
		return false;
	}
	
	public void doAddTag(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		if(name==null||name.equals("")||photoId==null||photoId.equals("")){
		...
		if(tag.getTid()>0){ 
			...
			if(phtDao.checkPhotoHasTag(name)){
